...
export class ImageOptimizationService {
    ...
    generateResponsiveImageSet(originalUrl: string, alt: string): ResponsiveImageSet {
        ...
        const srcset = sizes.map(({ zoom, descriptor }) => {
     ...
     */
    generatePlaceholder(width: number = 256, height: number = 384): string {
     ...
     */
    getLoadingPriority(index: number, isVisible: boolean = false): 'high' | 'low' {
